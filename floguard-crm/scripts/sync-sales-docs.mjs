 ...
 *
 * Branded PDF editions live in Sales Docs/pdf/ (generated by
 * scripts/build_sales_pdfs.py — rerun it after editing any md doc) and are
 * cataloged automatically next to their markdown source.
 *
...
function main() {
  ...
  for (const entry of MANIFEST) {
    ...
    // Branded PDF edition (Sales Docs/pdf/<stem>.pdf) — catalog beside the md
    const stem = path.basename(entry.file, path.extname(entry.file));
    const pdfFrom = path.join(srcDir, 'pdf', `${stem}.pdf`);
    if (fs.existsSync(pdfFrom)) {
      const pdfDest = path.join('pdf', `${stem.replace(/\s+/g, '_')}.pdf`).replace(/\\/g, '/');
      copyFileSafe(pdfFrom, path.join(destDir, pdfDest));
      const pdfStat = fs.statSync(path.join(destDir, pdfDest));
      pushItem(items, {
        destName: pdfDest,
        title: `${entry.title} — PDF`,
        category: entry.category,
        description: `Print-ready branded PDF. ${entry.description}`,
        type: 'pdf',
        stat: pdfStat,
      });
      copied += 1;
      console.log('OK', pdfDest);
    }
  }
